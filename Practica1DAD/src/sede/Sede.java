...
public class Sede {
	...
	/**
	 * Metodo que te muestra el menu con lo que puedes hacer en la sede de forma local
	 * @deprecated
	 */
 ...
 */
	public Sede() {
		...
		if(!this.cargarBaseDatos())System.out.println("ERRROR CARGANDO LA BASE DE DATOS");
		Usuario admin = new Usuario("admin","admin");
		this.addUser(admin);
	}

	/**
	 * Metodo main de la clase sede, se usa para efectuar pruebas locales (sin servidor)
	 * @param args
	 * @deprecated
	 */
 ...
 */
	public static String funcionalidadSede(Sede sede, Scanner sc) {
		...
		switch(respuesta){
			...
			case "1":
				...
				if(sede.addUser(tmpUser))System.out.println("Usuario añadido correctamente");
			...
			case "3":
				...
				nombre = sc.nextLine();
				if(sede.removeUsuario(nombre))System.out.println("Usuario borrado correctamente");
			...
			case "5":
				...
				if(!sede.addCliente(tmpCliente))System.out.println("No se puede añadir el cliente");
				else System.out.println("Cliente añadido correctamente");
				break;
			case "6":
			...
			case "7":
				...
				if(sede.removeCliente(id))System.out.println("Cliente borrado correctamente");
	...
	public boolean addUser(Usuario user) {
		if(this.usuarios.containsKey(user.getUser()))return false; //si el hashtable contine el nombre del usuario como llave devuelve false
	...
	/**
	 * Metodo para mostrar los datos de todos los usuarios guardados (Metodo de permisos de admin)
	 */
	...
	public boolean removeUsuario(String nombre) {
		if(!this.usuarios.containsKey(nombre))return false;
	 ...
	 * @return false si ya había un cliente en la posicion en la que se va a añadir el cliente (no deberia ocurrir)
	 */
	public boolean addCliente(Cliente cliente) {
		int key = generateClientKey();
		if(this.buscarCliente(key))return false;
		clientes.put(key,cliente);
		return true;	
	}

	/**
	 * Metodo algo pocho para encontrar una clave libre de cliente, se ejecuta al crear un cliente
	 * @return clave que asignar al nuevo cliente
	 */
	public int generateClientKey() {
		for(int i=0;i<Integer.MAX_VALUE;i++) {
			if(!this.buscarCliente(i)) return i;
		}
		return clientes.size()+1;
	}
	
	/**
	 * Metodo para mostrar el nombre de todos los clientes guardados (Metodo de permisos de gerente)
	 */
	 ...
	 * @return false si no ha sido capaz de hacerlo,true si se ha guardado correctamente;
	 *
	 */
	public boolean guardarBaseDatos() {
	        if(!guardarFichero("usuarios.ser",this.usuarios))return false;
	       if(!guardarFichero("clientes.ser",this.clientes))return false;
	...
	public Usuario inicioSesion(String nombre, String pass) {
		if(!this.buscarUser(nombre)) return null;
		Usuario user = this.usuarios.get(nombre);
		if(user.validatePass(pass))return user;
		return null;
	}
